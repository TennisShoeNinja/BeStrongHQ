...
import { HeaderMaxes } from "@/components/athlete-detail/HeaderMaxes";
...
const TABS: { label: string; href: string; segment: string | null }[] = [
  { label: "Overview", href: "", segment: null },
  { label: "PR History", href: "history", segment: "history" },
  { label: "Body", href: "body", segment: "body" },
  { label: "Meets", href: "meets", segment: "meets" },
  { label: "Programs", href: "programs", segment: "programs" },
];
...
export default function AthleteDetailLayout({
  ...
  // Single OPL link/manage dialog. OpenPowerlifting is the canonical source
  // for meet history now.
  ...
  // Hydrate edit/availability/program-due form state whenever the athlete
  // record changes (derived-from-props pattern, no effect needed).
  ...
  const bumpDate = (bumpUnit: 'week' | 'month') => {
  ...
  return (
    <>
      ...
      <div className="flex flex-col" style={{ gap: "var(--cloud-s4)" }}>
              ...
              {(() => {
                const parts = [
                  ...
                  athlete.weight_class
                    ? `${athlete.weight_class} class`
                    : null,
                  athlete.division || null,
                  athlete.age != null ? `Age ${athlete.age}` : null,
                  lastSyncedRelative ? `Synced ${lastSyncedRelative}` : null,
        ...
        {/* Context bar: status pills + unit toggle + comp-maxes toggle. Persists across tabs. */}
        <div className="flex flex-wrap items-center justify-between gap-3" style={{ marginTop: "calc(var(--cloud-s2) * -1)", paddingBottom: "var(--cloud-s3)", borderBottom: "1px solid var(--cloud-border)" }}>
          <ProfileStatusPills
            athlete={athlete}
        ...
        {/* Persistent maxes strip (above tabs, shows on every tab) */}
        <HeaderMaxes athleteId={athleteId} />

        {/* Tab nav strip */}
        <nav className="cloud-detail-tabs" aria-label="Athlete sections">
          {TABS.map((tab) => {
